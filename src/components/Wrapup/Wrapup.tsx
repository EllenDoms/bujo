...
import { getOpenBulletsWithStatus } from '../../supabase/bullets.store';
import { IBulletWithStatus } from '../../types/bullets';
...
export function Wrapup({ onClose }: Props) {
  ...
  const [initialOpenBulletStatus, setInitialOpenBulletStatus] = useState<IBulletWithStatus[]>();
  const [selectedIndex, setSelectedIndex] = useState<number>(0);
  const selectedBullet = initialOpenBulletStatus && initialOpenBulletStatus[selectedIndex];

  useEffect(() => {
    getOpenBulletsWithStatus(subDays(new Date(), 1)).then((res) => {
      res && setInitialOpenBulletStatus(res);
    });
  }, []);
          ...
          <div className="items-center flex flex-col text-center ">
            <div className="text-xs w-full">
              <DotCounter
                ...
                total={initialOpenBulletStatus.length}
              />
            </div>
